...
/**
 * Non-interactive checkbox visual (box + check). For rows that own their own
 * interactive control (e.g. a `<button>` carrying `aria-pressed`), where a real
 * nested checkbox would be invalid. Always `aria-hidden`.
 */
export function CheckboxMark({ checked = false, className }: { checked?: boolean; className?: string }) {
  return (
    <span
      aria-hidden="true"
      className={cn(
